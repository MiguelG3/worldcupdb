...
echo "$($PSQL "SELECT name FROM teams WHERE team_id IN (SELECT opponent_id FROM games WHERE round='Eighth-Final' AND year=2014) OR team_id IN (SELECT winner_id FROM games WHERE round='Eighth-Final' and year=2014) ORDER  BY name")"
